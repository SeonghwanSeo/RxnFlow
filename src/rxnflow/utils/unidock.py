...
def unidock_scores(
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        runner = UniDock(
            Path(pocket_file), sdf_list, center[0], center[1], center[2], size[0], size[1], size[2], workdir=root_dir
        )
        runner.docking(out_dir, search_mode=search_mode, num_modes=1, seed=seed)
        for docked_sdf_file in (root_dir / "docking_dir" / "docking_results").iterdir():
