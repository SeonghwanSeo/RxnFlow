...
class SynthesisGFN_DP(nn.Module):
    ...
    def __init__(
        ...
        self.transf = gt.GraphTransformer(
        ...
        self._action_type_to_num_inputs_outputs = {
            ...
            ReactionActionType.ReactBi: (num_glob_final + env_ctx.num_bimolecular_rxns * 2, num_emb_block),
        }
    ...
    def hook_reactbi(
        ...
        logits = torch.matmul(mlp(torch.cat([emb, rxn_features], -1)), block_emb.T)  # [Ngraph, Nblock]
    ...
    def single_hook_add_first_reactant(self, emb: torch.Tensor, block_emb: torch.Tensor):
        ...
        """
        mlp = self.mlps[ReactionActionType.AddFirstReactant.cname]
        return torch.dot(mlp(emb), block_emb)
    ...
    def single_hook_reactbi(self, emb: torch.Tensor, rxn_id: int, block_is_first: bool, block_emb: torch.Tensor):
        ...
        rxn_features[rxn_id * 2 + int(block_is_first)] = 1
        return torch.dot(mlp(torch.cat([emb, rxn_features], -1)), block_emb)
