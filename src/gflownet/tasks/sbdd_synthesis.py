...
class SBDDSampler(SynthesisGFNSampler):
    ...
    def setup_task(self):
        ...
        self.task.models["proxy"].to(self.device)
...
def default_config(env_dir: str | Path, pocket_db: str | Path, proxy: str, proxy_dataset: str) -> Config:
    assert proxy in ["unidock", "qvina"]
    assert proxy_dataset in ["CrossDocked2020", "ZINCDock15M"]
    config = init_empty(Config())
    config.desc = "Proxy-QED optimization with proxy model"
    config.env_dir = str(env_dir)
    config.task.sbdd.pocket_db = str(pocket_db)
    config.task.sbdd.proxy = f"{proxy}-{proxy_dataset}"
